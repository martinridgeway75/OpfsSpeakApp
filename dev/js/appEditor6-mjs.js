...
function initApp() {
    ...
    onValue(ref(db, path), (snapshot) => {
        const infoObj = snapshot.val();

        if (infoObj != null) {
            //if the browser is refreshed while the user is logged in, then the signIn button can remain hidden
            //...and the keyDown & keyUp Listeners will remain active, so:
            if ( docEl("leSignout").classList.contains("nodisplay") ) {
                preInit();
            }
            resetAppEditor(infoObj.pNym, infoObj.ctx);
            getRecordsIndexFromDbAtInit();
        } else {
            chkPermission("Your information doesn't exist on the database.");
        }
    }, (error) => {
        chkPermission(error);
    }, {
...
function preInit() {
    ...
    showEl("leSignout");
    appEditor = {};
    signInBtn.addEventListener('click', toggleSignIn, {capture:false,passive:true});
}
